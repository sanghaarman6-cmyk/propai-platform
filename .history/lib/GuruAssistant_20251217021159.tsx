...
export default function GuruAssistant({ pageHint }: { pageHint?: string }) {
  ...
  const chips = useMemo(() => buildContextChips(activeChallenge), [activeChallenge])
    ...
    <div className="h-full">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          ...
          className="text-xs text-text-muted hover:text-white hidden lg:inline-flex"
        >
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        ...
        {pageHint && (
          <TagPill tone="neutral">You are in: {pageHint}</TagPill>
        )}
      </div>

      <div className="rounded border border-border bg-black/30 p-3 h-[52vh] overflow-y-auto">
      ...
      <div className="mt-3">
        <div className="text-xs text-text-muted mb-2">Suggested prompts</div>
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button
          onClick={() => send(input)}
          className="rounded bg-accent-green px-3 py-2 text-sm text-black hover:shadow-glow"
        >
          Send
        </button>
      </div>
        ...
        >
          {desktopCollapsed ? (
            <div className="h-full flex flex-col items-center justify-between">
              <div className="text-xs text-text-muted font-mono rotate-90 mt-10">
                GURU
              </div>
              <button
