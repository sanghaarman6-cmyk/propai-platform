  ...
  try {
    ...
    const response = await openai.responses.create({
        ...
        input: [
            ...
            If the account status is "Failed", you must also include a failure_analysis object.

            Rules for failure analysis:
            - Identify the MOST likely primary reason
            - Secondary factors can include behavior, timing, or risk
            - Psychological pattern should be inferred carefully
            - preventable should be true unless failure was purely statistical
            - ai_verdict should be a short, direct mentor-style judgment

            If the account is not failed, omit failure_analysis.
