...
export default function DashboardPage() {
  ...
  // ✅ ALWAYS define fallbacks so hooks still run
  ...
  /* ---------- METRICS (HOOK SAFE) ---------- */
  const { ddTotalPct, ddTodayUsd } = computeDrawdownMetrics(
    history,
    balance
  )
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-6">
        ...
        <KPI
          ...
          accent
        />
        <KPI label="Drawdown" value={`${ddTotalPct.toFixed(2)}%`} />
        <KPI
      ...
      <TerminalCard title="Risk Overview">
        <RuleMeter
          ...
          valuePct={ddTotalPct}
          helper={`${ddTodayUsd.toFixed(2)} ${currency}`}
        />
      </TerminalCard>
